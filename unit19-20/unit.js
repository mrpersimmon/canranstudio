(function (root) {
  'use strict';
  const core = root.CanranCore, unit = core.unit1920;
  ...
  function mountPractice(id, settings = {}) {
    const element = node('div'); element.id = 'unit1920-' + id + '-practice'; surfaces.get(id).append(element);
  ...
  const cast = node('div', '', 'dialogue-cast'); cast.append(actor('girl'), actor('boy'));
  stage.append(actor('mother'), log, cast);
  ...
  function refreshDialogue() {
    ...
    stage.querySelectorAll('.dialogue-actor').forEach(person => person.classList.toggle('is-current', speaker === person.dataset.actor || (speaker === 'children' && ['girl', 'boy'].includes(person.dataset.actor))));
  }
  function lead() { const lead = node('div', '', 'story-lead'); lead.append(art('children'), node('p', '孩子们为什么向妈妈道谢？')); log.replaceChildren(lead); }
  ...
  function renderWords() {
    ...
    for (const word of content.WORDS.slice(wordPage * 6, wordPage * 6 + 6)) {
      ...
      pronunciation.id = 'u1920-' + word.en.replaceAll(' ', '-') + '-phonetic';
      meaning.id = 'u1920-' + word.en.replaceAll(' ', '-') + '-meaning';
  ...
  const usage=node('details','','offline-task');usage.append(node('summary','看对象，选意思'),
    node('p','old 形容人是年老的，形容帽子是旧的。short 形容身高是矮的，形容裤子长度是短的。light 描述箱子的重量时是轻的。'),
    node('p','grandfather 可以指祖父或外祖父；grandmother 可以指祖母或外祖母。'),
    node('p','Look at them! 中 them 指前面说到的多个人或物品。They 可以放在句首作主语；at 后面用 them。'));
  const context=node('details','','offline-task');context.append(node('summary','把话放回故事里'),
    node('p',"What's the matter? 问怎么了；all right 在这里表示状态好。nice 形容课文的冰淇淋，是好吃的。"),
    node('p',"There's = There is；这里说有一个卖冰淇淋的人。two ice creams 指两份冰淇淋。"),
    node('p','休息不等于懒惰。胖瘦、高矮、年龄只是不同特点，不代表一个人的好坏。'));
  surfaces.get('phrases').append(phraseGrid,usage,context,phraseActions);
  const galleryDetails=node('details','','offline-task visual-gallery');galleryDetails.append(node('summary','看看二十幅对比图'));
  ...
  const models=node('details','','offline-task');models.append(node('summary','看看完整问答'),node('p','这些是新的练习情境，按每组给出的状态回答。'));
  ...
  const reference=node('details','','offline-task');reference.append(node('summary','am、is 还是 are？'),node('p','例：Those children are thirsty.'));
  ...
  const forms=node('details','','offline-task');forms.append(node('summary','把句子说准确'),
    node('p','child → children；shoe → shoes。Their mother 指他们的一位妈妈，用 is；those children 指多个孩子，用 are。'),
    node('p',"They're clean. 中的 clean 是形容词，不因主语复数而加 s。"),
    node('p',"They're not dirty. 和 They aren't dirty. 都正确。只说不脏，并不能自动说明所有其他特点；要按看到或读到的信息描述。"));
  const numbers=node('details','','offline-task');numbers.append(node('summary','跟老师读数字'),node('p','105 · 106 · 217 · 218 · 321 · 322 · 433 · 434 · 545 · 546 · 657 · 658 · 769 · 770 · 881 · 882 · 998 · 999 · 1,000 · 1,001'));
  ...
  certificateView = core.unitCertificate.mount({
    ...
    design: {
      copy: { title: '细心的小小观察员', course: '冰淇淋休息站 · Lesson 19–20', completion: '完成 Lesson 19–20 课堂配套练习', thanks: '留意小伙伴，也发现身边事物的变化！' },
      characters: ['mother', 'children'], characterLabels: ['妈妈', '孩子们'], icon: art, defaultName: '细心的小小观察员', dialogTitle: '冰淇淋休息站纪念', fileName: 'Lesson19-20-冰淇淋休息站.png',
  ...
  writing.append(node('summary','和朋友再试试'),node('p','跟老师听辨状态词、带读课文。两人交换关心者与孩子的角色，问 What’s the matter?，再根据状态卡回答。用大小、长短、轻重的实物观察，不凭大小猜重量。'));
  writing.append(node('h4','Lesson 19–20 · 纸笔小练习'),node('p','A．用 am、is 或 are 填空。例：Those children are thirsty.'));
  ...
  writing.append(node('p','B．按每组给出的新情境，仿照例子提问并回答。括号内是需要否定的状态。'),node('p',"例：his shoes / (dirty) / clean → Are his shoes dirty or clean? They're not dirty. They're clean."));
  const replyWriting=node('ol','','reply-writing');content.MODELS.forEach(item=>{const row=node('li');row.append(node('span',item.subject+' / ('+item.wrong+') / '+item.right));for(let i=0;i<6;i++)row.append(node('span','','writing-rule'));replyWriting.append(row);});writing.append(replyWriting);
