...
async function main(){
 ...
 try{
  ...
  try {
   ...
   const hear=async()=>{await page.locator('.lp-preparation').waitFor({state:'hidden'});for(let i=0;i<40;i++){await page.waitForFunction(()=>fixture.runtime.snapshot().audio?.status!=='loading');const s=await state();if(s.audio?.status!=='playing')break;const before=await page.evaluate(()=>fixture.completedDispatches['audio-ended']||0);await page.evaluate(()=>fixture.audio.at(-1).finish());await page.waitForFunction(before=>(fixture.completedDispatches['audio-ended']||0)>before,before);}};
