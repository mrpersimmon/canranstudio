...
function hasSameOriginAudioReference(source, course) {
  const recordings = course.learning?.AUDIO;
  if (recordings && source.includes('learning.AUDIO')) {
    const paths = Object.values(recordings).filter(Boolean);
    return paths.length > 0 && paths.every(asset =>
      typeof asset === 'string' && asset.startsWith(`${course.id}/audio/`) &&
      !asset.includes('..') && asset.endsWith('.mp3')) &&
      course.learning.DIALOGUE.every(line => Boolean(recordings[line.text]));
  }
  const tokens = [
    "'audio/", '"audio/', '`audio/',
