...
async function main(){
  ...
  try{
    ...
    assert.equal(await page.locator('[data-action="journey-section"],[data-action="journey-expand"]').count(),0);
    ...
    const sampleRefs=['L01-D01','L01-W05','L16-W03','L07-D01','L21-D01','L25-D01','L27-NUM10','L31-D04','L33-D08','L35-D03','L37-D03','L39-D08','L41-D06','L43-D01','L45-D03','L47-D01','L49-D15','L49-ORD01',
    ...
    sampleRefs.push(...Object.keys(unit.sources).filter(ref=>ref.startsWith('G49-')));
    ...
    const migrated=await p.evaluate(key=>window.CanranCore.learningStore.createLocalStorageAdapter(localStorage).load(key).value,old.rt.storageKey);
    assert.deepEqual(migrated.keyboardArchive.record,oldRecord.value);
