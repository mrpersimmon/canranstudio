(function (root) {
  ...
  const learning = {...base, WORDS:[...base.WORDS,...foods], AUDIO:{...base.AUDIO}, SUBJECTS:{...base.SUBJECTS, version:'unit49-50-subjects-v2', reviewMode:'mistakes-only', questions:base.SUBJECTS.questions.filter(q=>q.id!=='S04')}};
  ...
  const questions = {
    listen:listening,
    roles:base.STORY_TASKS.filter(q=>q.id!=='story-steak-too'),
    doare:[...take(base.DOARE,['doare-like','doare-teacher']),
      ...
      question('v2-does-want','区分本次需求与平时喜好','你正在和 Sam 谈论 Penny。想问 Sam 她今天要不要桃子，应该怎样说？',['Does she want peaches?','Does she like peaches?','Do you want peaches?'],'Does she want peaches?','今天要不要用 want；she 指正在谈论的 Penny。like 问喜好，you 会问成 Sam 的需求。',{hint:'先看是在问谁，再分清“喜欢”还是“这次想要”。'})],
    ...
    either:take(base.ET,['either-form-too','either-form-either','either-home']).map(q=>q.id==='either-form-too'?{...q,explanation:q.explanation+' 喜好不同时，直接说自己的喜好，不用 too 表示相同。'}:q),
    ...
    exam:[...take(base.EXAM,['exam-listen','exam-story']),
      question('v2-exam-shopping','整合两人的本次需求',"Penny: I like tomatoes, but I don't want any.\nSam: I want some cabbage.\n按两人的话，这次应该买什么？",['cabbage','tomatoes','tomatoes and cabbage'],'cabbage','Penny 本次不要西红柿；Sam 想要卷心菜，所以这次买 cabbage。',{hint:'喜欢不等于这次想要；分别看两人的需求。'}),
      question('v2-exam-different','按不同喜好如实回应',"A: I don't like chicken.\nB 喜欢鸡肉。B 应该怎样如实回应？",['I like chicken.',"I don't like chicken either.",'I like chicken, too.'],'I like chicken.','两人喜好不同。B 直接说 I like chicken.；too 表示也喜欢，either 表示也不喜欢，这里都不能用来表示与 A 相同。',{hint:'too／either 表示相同的情况；不同就说自己的喜好。'}),
      question('v2-exam-edit','在否定句中检查动词原形',"Sam doesn't likes grapes.\n只改一处，让这句话正确。",['likes 改为 like',"doesn't 改为 don't",'grapes 改为 grape'],'likes 改为 like',"Sam doesn't like grapes. doesn't 后用 like 原形；Sam 是第三人称单数，不能改成 don't。",{hint:"保留 Sam 和否定意思，检查 doesn't 后面的动词。"})]
