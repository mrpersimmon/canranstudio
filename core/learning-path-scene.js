  ...
  function createRenderer(unit) {
    ...
    function vocabulary(a, v) {
      return `${a.guideRef ? `<div class="lp-phrase-example"><p lang="en">${escape(unit.sources[a.guideRef].text)}</p>${a.guideCaption ? `<small>${escape(a.guideCaption)}</small>` : ''}</div>` : ''}<div class="lp-vocabulary" role="group" aria-label="${escape(a.title)}">${a.items.map(item => {
        ...
        return `<button type="button" class="lp-vocabulary-card${active ? ' is-speaking' : ''}${heard ? ' is-heard' : ''}" data-action="word-play" data-id="${escape(item.sourceRef)}" aria-label="${escape(`${c.wordListen} ${unit.sources[item.sourceRef].text}`)}"${active ? ' aria-current="true"' : ''}>${image(item.entityId, 'lp-vocabulary-image')}<span class="lp-vocabulary-caption"><strong lang="en">${escape(item.term || unit.sources[item.sourceRef].text)}</strong><span>${escape(item.caption || unit.entities[item.entityId].title)}</span></span>${heard ? `<span class="lp-word-heard" aria-label="${escape(c.wordHeard)}">${icon('check-circle-fill')}</span>` : ''}<span class="lp-word-cue" aria-hidden="true">${icon('volume-up-fill')}<span>${escape(active ? c.wordPlaying : heard ? c.replay : a.audioType === 'sentence' && c.listenExample ? c.listenExample : c.wordListen)}</span></span></button>`;
      }).join('')}</div>`;
    }
    ...
    function render(v) {
      ...
      if (v.screen === 'map') html = renderMap(v);
        ...
        if (v.storyActivityId) {
          ...
          body = vocabulary(a, v);
          actions = `${['playing', 'paused'].includes(v.audio?.status) ? playback(v) : ''}<p class="lp-word-progress">${v.heardWords.length} / ${a.items.length}</p>${button('continue', c.next, { disabled: !v.canContinue, className: 'lp-primary' })}`;
        } else {
