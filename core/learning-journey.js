(function attach(root, factory) {
  ...
  function describe(unit, view) {
    ...
    const currentId = view.nodes.find(node => !(node.passed || node.done))?.id || null;
    return {
      currentId,
      ...
      nodes: unit.nodes.map((node, index) => ({ ...node, ...states.get(node.id), index, current: currentId === node.id,
        canJump:Boolean(unit.placement && !states.get(node.id)?.available && unit.nodes.find(n=>n.chapterId===node.chapterId)?.id===node.id),
      ...
      chapters: unit.chapters.map(chapter => ({ ...chapter, total: unit.nodes.filter(node => node.chapterId === chapter.id).length, done: unit.nodes.filter(node => node.chapterId === chapter.id && (states.get(node.id)?.passed || states.get(node.id)?.done)).length }))
    };
  }

  function render(unit, view) {
    const j = unit.journey, c = j.copy, model = describe(unit, view);
    const passedCount=view.passedCount??view.completedCount, hasSkips=passedCount>view.completedCount;
    ...
    const stats = `<header class="journey-stats"><span class="journey-stat" aria-label="新概念英语第 ${unit.lessonIds[0]} 至 ${unit.lessonIds.at(-1)} 课">${icon('book-open','tone-blue')}<b>${escape(range)}</b></span><span class="journey-stat tone-gold" aria-label="${hasSkips ? '已通过' : '已完成'} ${passedCount} 关，共 ${unit.nodes.length} 关">${icon('star','tone-gold')}<b>${passedCount}<small> / ${unit.nodes.length}</small></b></span><button type="button" class="journey-stat tone-pink" data-action="journey-nav" data-id="review" aria-label="${view.dueCount} 条待复习线索">${icon('arrows-clockwise','tone-pink')}<b>${view.dueCount}</b></button></header>`;
    function preview(node) {
      const challenge = unit.challenges?.find(item=>item.nodeIds.includes(node.id));
      const label = node.skipped ? '学习这一关' : node.done ? c.repeat : node.available ? node.started ? c.resume : c.start : `${c.first} ${node.index} ${c.level}`;
      return `<section class="journey-preview${node.available ? '' : ' is-locked'}" id="journey-preview-${node.id}" aria-label="${escape(node.title)}关卡介绍"><div class="journey-preview-heading"><span>${escape(lessonLabel(node.lessonIds))} · ${escape(j.nodeLabels[node.id])}</span>${action('journey-close','',{css:'journey-close',symbol:'x',aria:c.close})}</div><h2>${escape(node.title)}</h2>${node.canJump ? action('open-placement','准备开始跳级',{id:node.chapterId,css:'journey-primary journey-placement-primary',symbol:'fast-forward'}) : action('open-node', label, { id: node.id, disabled: !node.available, css: 'journey-primary', symbol: node.available ? 'play' : 'lock-simple' })}${node.skipped ? '<p class="journey-placement-note">跳级通过 · 还可以完整学习</p>' : ''}${challenge && (node.done || node.skipped) ? action('open-challenge','小镇挑战 · 选做',{id:challenge.id,css:'journey-outline journey-challenge-entry'}) : ''}</section>`;
    }
    function nodeMarkup(node, companion = '') {
      const state = node.canJump ? 'jump' : node.skipped ? 'skipped' : node.done ? 'done' : node.current ? 'current' : 'locked';
      const offsets = [0, 48, 72, 48, 0, -48, -72, -48];
      const x = offsets[node.index % offsets.length];
      const status = node.canJump ? '可以跳级到这里' : node.skipped ? '跳级通过' : node.done ? c.completed : node.available ? node.started ? c.resume : c.start : c.locked;
      const hint = node.canJump && !selected ? action('open-placement', view.record.placement?.attempts[node.chapterId]?.status==='active' ? '继续跳级测试' : '跳级到这儿？', {id:node.chapterId,css:'journey-start-hint journey-jump-hint'}) : node.current && !selected ? action('preview-node', node.started ? c.resume : c.start, { id: node.id, css: 'journey-start-hint' }) : '';
      const surface = `${node.canJump || node.skipped ? '<span class="journey-jump-face" aria-hidden="true"></span>' : ''}<img class="journey-coin" src="${escape(node.done ? j.assets.gold : j.assets.orange)}" alt="" width="92" height="86">${icon(node.canJump || node.skipped ? 'fast-forward' : j.nodeIcons[node.id], 'journey-node-symbol')}`;
      return `<li class="journey-step is-${state}${selected === node.id ? ' is-selected' : ''}${companion ? ' has-companion' : ''}" style="--node-x:${x}px" data-journey-node="${node.id}"${selected && selected !== node.id ? ' inert' : ''}>${hint}<button type="button" data-action="preview-node" data-id="${node.id}" class="journey-node" aria-label="第 ${node.index + 1} 关 ${escape(node.title)}，${escape(lessonLabel(node.lessonIds))}，${escape(j.nodeLabels[node.id])}，${status}" aria-expanded="${selected === node.id}"${selected === node.id ? ` aria-controls="journey-preview-${node.id}"` : ''}${node.current ? ' aria-current="step" data-journey-current' : ''}>${node.current ? `<span class="journey-ring" aria-hidden="true"></span>` : ''}${surface}${node.skipped ? '<span class="journey-skipped-label">跳级通过</span>' : ''}${node.done ? `<span class="journey-done-badge">${icon('check')}</span>` : ''}</button>${selected === node.id ? preview(node) : ''}${companion}</li>`;
    }
    function route() {
      return `${unit.chapters.length > 3 ? `<details class="journey-lesson-index"><summary>课程目录 · Lesson ${escape(range)}</summary><nav aria-label="选择 Lesson 分区">${unit.chapters.map(ch => `<a href="#chapter-${escape(ch.id)}">${escape(lessonLabel(ch.lessonIds))}<span>${escape(ch.title)}</span></a>`).join('')}</nav></details>` : ''}${model.chapters.map((chapter, index) => {
        ...
        const companion = companionNode ? `<div class="journey-companion ${right ? 'on-left' : 'on-right'}"><button type="button" data-action="journey-greet" class="journey-mascot" aria-label="${escape(c.greet)}"><img src="${escape(j.assets.cat)}" alt="挥手的探险猫" width="148" height="222" decoding="async"></button><span class="journey-greeting" role="status" hidden>${escape(completed ? c.doneHello : c.hello)}</span><div class="journey-chapter-stars" aria-label="本主题已通过 ${chapter.done} 关，共 ${chapter.total} 关">${nodes.map(node => icon('star',node.passed || node.done ? 'tone-gold' : 'tone-locked')).join('')}</div></div>` : '';
      ...
      }).join('')}<div class="journey-finish">${icon('trophy',passedCount === unit.nodes.length ? 'tone-gold' : 'tone-locked')}<p>${escape(passedCount === unit.nodes.length ? hasSkips ? '第一册路线已通过！' : c.allDone : c.footer)}</p></div>${model.currentId ? action('journey-locate','',{css:'journey-locate',symbol:'arrow-up',aria:c.current}) : ''}`;
    }
    function challengeList() {
      return `<h2 class="journey-section-title">小镇挑战 · 选做</h2><div class="journey-replay-list">${(unit.challenges || []).map(challenge => {
        const parent = model.nodes.find(node=>node.id===challenge.unlockNodeId), unlocked=parent?.done || parent?.skipped;
        const progress = view.record.challenges?.[challenge.id];
        const label = unlocked ? `${challenge.title} · ${progress?.completedAt ? '已完成' : (progress?.legacyCount||progress?.answers.length) ? '继续' : '开始'}` : `${challenge.lessonLabel} · 完成第 ${unit.nodes.findIndex(n=>n.id===challenge.unlockNodeId)+1} 关后解锁`;
    ...
    function historicalProgress(){
      const old=view.record.keyboardArchive?.record;if(!old)return '';
      const answers=Object.values(old.challenges||{}).flatMap(c=>c.answers),results=Object.values(old.results||{}).filter(r=>typeof r.value==='string'),attempts=Object.values(old.placement?.attempts||{});
      if(!answers.length&&!results.length&&!attempts.length)return '';
      return `<details class="journey-history"><summary>查看以前的练习记录</summary><p>以前完成的练习继续保留。新练习会单独记录。</p><ul>${[...results.map(r=>({value:r.value,at:r.at||r.completedAt||''})),...answers].map(a=>`<li><span lang="en">${escape(a.value)}</span><small>${escape(a.at.slice(0,10))}</small></li>`).join('')}${attempts.map(a=>`<li>跳级到 ${escape(unit.chapters.find(c=>c.id===a.targetId)?.title||a.targetId)}：${a.status==='passed'?'已通过':a.status==='failed'?'当次未通过':'因版本更新中止，可重新开始'}（${a.responses.length} / 20 题）</li>`).join('')}</ul></details>`;
    }
    function grammarProgress() {
      if(!unit.grammar)return '';
      return `<section class="journey-panel"><h2 class="journey-section-title">语法练习记录 · Lesson 49–50</h2><p>记录选词、组句、改错与隔日复习的表现。一次答对还需要后续练习验证。</p><div class="journey-chapter-records">${unit.grammar.skills.map(skill=>{
        const results=Object.values(view.record.results).filter(r=>r.assessment?.grammarSkillId===skill.id && r.contractVersion===3);
        const independent=results.filter(r=>r.initialEvidence==='independent').length,supported=results.length-independent;
        const transfer=results.filter(r=>r.assessment.phase==='transfer' && r.initialEvidence==='independent').length;
        const delayed=Object.values(view.record.retrievalReviews || {}).filter(e=>e.grammarSkillId===skill.id&&e.lastEvidence==='independent').length;
        return `<div><strong>${escape(skill.label)}</strong><p>未用额外提示答对 ${independent} · 借助帮助 ${supported}<br>新句未用额外提示答对 ${transfer} · 隔日未用额外提示答对 ${delayed}</p></div>`;
      }).join('')}</div></section>`;
    }
    function progress() {
      return `<section class="journey-panel"><p class="journey-eyebrow">${escape(c.progress)}</p><h1 tabindex="-1" data-lesson-title>${escape(c.progressTitle)}</h1><p class="journey-panel-intro">${escape(c.progressSubtitle)}</p><div class="journey-metrics">${[['star','gold',view.completedCount+' / '+unit.nodes.length,c.completedLabel],['headphones','blue',model.heard+' / '+unit.dialogueRefs.length,c.heardLabel],['check','mint',model.practiced,c.practiceLabel],['arrows-clockwise','pink',view.dueCount,c.dueLabel]].map(([symbol,tone,value,label]) => `<div>${icon(symbol,'tone-'+tone)}<strong>${value}</strong><span>${escape(label)}</span></div>`).join('')}</div>${hasSkips ? `<p class="journey-placement-note">${passedCount-view.completedCount} 关通过跳级解锁。课文听读与答题记录只统计实际完成的内容。</p>` : ''}<h2 class="journey-section-title">${escape(c.chapterLabel)}</h2><div class="journey-chapter-records">${model.chapters.map(chapter => `<div><span>${escape(chapter.title)}</span><b>${chapter.done} / ${chapter.total}</b><progress max="${chapter.total}" value="${chapter.done}" aria-label="${escape(chapter.title)}完成进度"></progress></div>`).join('')}</div>${view.completedCount ? '' : `<p class="journey-empty">${escape(c.noResults)}</p>`}${grammarProgress()}${historicalProgress()}<section class="journey-reset"><h2>重新开始</h2><p>仅影响这台设备的学习记录。</p>${action('reset-request','重置小镇挑战',{id:'challenges',css:'journey-outline'})}${action('reset-request','全部从零开始',{id:'course',css:'journey-outline'})}</section></section>`;
    }
    return `<div class="journey-app" data-journey-tab="${tab}">${navigation}<div class="journey-main">${stats}${view.record.resetBackup ? `<section class="journey-reset-notice" role="status"><p>已重置，可在再次学习前撤销。</p>${action('reset-undo','撤销重置',{css:'journey-outline'})}</section>` : ''}${tab === 'path' ? route() : tab === 'review' ? reviews() : progress()}</div><aside class="journey-desktop-note">${icon('compass','tone-orange')}<h2>猫猫小镇</h2><p>新概念英语 · Lesson ${escape(range)}</p><div><strong>${passedCount}</strong><span> / ${unit.nodes.length} 关${hasSkips ? '已通过' : '已完成'}</span></div><progress value="${passedCount}" max="${unit.nodes.length}" aria-label="总关卡进度"></progress><p>和探险猫一起，一步步向前。</p></aside></div>`;
  }
