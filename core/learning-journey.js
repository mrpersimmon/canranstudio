(function attach(root, factory) {
  ...
  function render(unit, view) {
    ...
    function nodeMarkup(node) {
      ...
      return `<li class="journey-step is-${state}${selected === node.id ? ' is-selected' : ''}" style="--node-x:${x}px" data-journey-node="${node.id}">${hint}<button type="button" data-action="preview-node" data-id="${node.id}" class="journey-node" aria-label="第 ${node.index + 1} 关 ${escape(node.title)}，${escape(j.nodeLabels[node.id])}，${status}" aria-expanded="${selected === node.id}"${selected === node.id ? ` aria-controls="journey-preview-${node.id}"` : ''}${node.current ? ' aria-current="step" data-journey-current' : ''}>${node.current ? `<span class="journey-ring" aria-hidden="true"></span>` : ''}${surface}${node.done ? `<span class="journey-done-badge">${icon('check')}</span>` : ''}</button>${selected === node.id ? preview(node) : ''}</li>`;
    }
    function route() {
      return `${model.chapters.map((chapter, index) => {
        ...
        const right = [0,1,2,3].includes(nodes[0].index % 8);
        return `<section class="journey-chapter${active ? ' is-active' : ''}" aria-label="${escape(chapter.title)}"><header class="journey-chapter-banner"><div><p>${escape(c.stage)} · 第 ${index+1} ${escape(c.part)}</p><h1${active || !model.currentId && index === 0 ? ' tabindex="-1" data-lesson-title' : ''}>${escape(chapter.title)}</h1></div>${action('journey-book','',{id:String(chapter.lessonIds[0]),css:'journey-book-button',symbol:'notebook',aria:c.bookLabel})}</header><div class="journey-trail"><ol class="journey-nodes" aria-label="${escape(chapter.title)}的关卡">${nodes.map(nodeMarkup).join('')}</ol>${active || completed ? `<div class="journey-companion ${right ? 'on-left' : 'on-right'}"><button type="button" data-action="journey-greet" class="journey-mascot" aria-label="${escape(c.greet)}"><img src="${escape(j.assets.cat)}" alt="挥手的探险猫" width="148" height="222" decoding="async"></button><span class="journey-greeting" role="status" hidden>${escape(completed ? c.doneHello : c.hello)}</span><div class="journey-chapter-stars" aria-label="本主题已完成 ${chapter.done} 关，共 ${chapter.total} 关">${nodes.map(node => icon('star',node.done ? 'tone-gold' : 'tone-locked')).join('')}</div></div>` : ''}</div></section>`;
