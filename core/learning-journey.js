(function attach(root, factory) {
  ...
  function render(unit, view) {
    ...
    function route() {
      const focusChapter=view.journeyUI?.chapterId || model.nodes.find(n=>n.id===(selected || model.currentId))?.chapterId || unit.chapters.at(-1).id;
      const center=Math.max(0,unit.chapters.findIndex(c=>c.id===focusChapter));
      const visible=view.journeyUI?.expanded?model.chapters:model.chapters.slice(Math.max(0,center-1),center+2);
      return `${unit.chapters.length > 3 ? `<details class="journey-lesson-index"><summary>课程目录 · Lesson ${escape(range)}</summary><nav aria-label="选择 Lesson 分区">${unit.chapters.map(ch => `<a href="#chapter-${escape(ch.id)}">${escape(lessonLabel(ch.lessonIds))}<span>${escape(ch.title)}</span></a>`).join('')}</nav></details>` : ''}${action('journey-expand',view.journeyUI?.expanded?'只看附近分区':'展开全部路线',{css:'journey-outline'})}${center>1&&!view.journeyUI?.expanded?action('journey-section','上一分区',{id:unit.chapters[center-2].id,css:'journey-outline'}):''}${visible.map((chapter, index) => {
      ...
      }).join('')}${center+2<unit.chapters.length&&!view.journeyUI?.expanded?action('journey-section','下一分区',{id:unit.chapters[center+2].id,css:'journey-outline'}):''}<div class="journey-finish">${icon('trophy',passedCount === unit.nodes.length ? 'tone-gold' : 'tone-locked')}<p>${escape(passedCount === unit.nodes.length ? hasSkips ? '第一册路线已通过！' : c.allDone : c.footer)}</p></div>${model.currentId ? action('journey-locate','',{css:'journey-locate',symbol:'arrow-up',aria:c.current}) : ''}`;
    }
    ...
    function grammarProgress() {
      ...
      return `<section class="journey-panel"><h2 class="journey-section-title">语法练习记录 · Lesson 49–50</h2><p>记录独立作答、借助帮助和隔日复习的表现。一次答对还需要后续练习验证。</p><div class="journey-chapter-records">${unit.grammar.skills.map(skill=>{
        const results=Object.values(view.record.results).filter(r=>r.assessment?.grammarSkillId===skill.id && r.assessment.evidenceMode==='written-retrieval');
