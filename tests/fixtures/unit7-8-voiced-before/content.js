(function(root){
 ...
 const images=Object.fromEntries(WORDS.map(w=>[w.en,w.image]));
 const listenWords=['Italian','keyboard operator','engineer','policeman','policewoman','taxi driver','air hostess','postman','nurse','mechanic','hairdresser','housewife','milkman'];
 const questions={
  listen:listenWords.map((en,i)=>{const others=listenWords.filter(x=>x!==en);return q('hear-'+i,'听辨 '+en,'听一听，选出单词。',[en,others[i%12],others[(i+4)%12],others[(i+8)%12]].map(v=>[v,'再听一次，分清 '+en+' 与 '+v+'。']),en,'再听一次，选出对应的词。','','本课新词','新词覆盖，不重复上单元八种国籍整组练习',{audioText:en,optionImages:images});}),
  roles:[
  ...
  be:[
   ...
   q('be-negative','姓名与代词的否定和肯定','Sophie ___ not Italian. She ___ French.',[['is / is',''],['am / is','Sophie 用 is，am 与 I 连用。'],['is / am','She 用 is，am 与 I 连用。']],'is / is','Sophie 和后面的 She 都指同一个人，用 is；not 只是说明否定。','这里没有 I；not 不会把 is 变成 am。','Lesson8 Written A2','核对否定与继续指代，完整A3留在线下和示范')
  ],
  interview:[
   ...
   q('ask-her','根据 her 对应人称并用 an','人物卡写着“她：air hostess”。\nIs ___ ___ air hostess?',[['she / an',''],['he / an','人物卡说她，用 she。'],['she / a','air 以元音开头，用 an。']],'she / an','Is she an air hostess?；根据人物卡用 she，air 前用 an。','先看人物，再听 air 的开头。','Lesson8 Written B4','把新的人物问法与一个已教冠词对比结合，不另刷十次')
  ],
  trans:[
   q('build-direct','完整职业问答','你问对方做什么工作；对方说自己是工程师。',undefined,"What's your job? I'm an engineer.",'直接问对方用 your；对方回答自己用 I’m。','先问 What’s your job?，再让对方回答。','Lesson7 D15–D16的角色换位','撤去完整英文答案，独立组织两个说话回合',{type:'order',tokens:["What's",'your job?',"I'm",'an engineer.']}),
  ...
  exam:[
   q('exam-record','同时听取国籍和职业','听一听，为这位新朋友选一份记录。',[['意大利人；护士',''],['法国人；护士','第一句说 Italian，不是 French。'],['意大利人；工程师','第二句说 nurse，不是 engineer。']],'意大利人；护士','Italian 介绍国籍，nurse 介绍职业；这位新朋友不是课文中的 Robert。','','新的完整录音情境','两项信息同时保持，不从已学原文人物套答案',{audioText:"I'm Italian. I'm a nurse."}),
 ...
 const stages=[
  {id:'l1',title:'采访前准备',activities:[['words','采访小图鉴','cards'],['listen','听音寻宝','audio']],required:['listen']},
