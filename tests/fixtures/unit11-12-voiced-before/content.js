(function(root){
 ...
 const listenWords=['whose','blue','perhaps','white','catch','father','mother','blouse','sister','tie','brother','his','her'];
 const images=Object.fromEntries(WORDS.map(w=>[w.en,w.image]));
 const questions={
  listen:listenWords.map((en,i)=>{const others=listenWords.filter(x=>x!==en);return q('hear-'+i,'听辨 '+en,'听一听，选出单词。',[en,others[i%12],others[(i+4)%12],others[(i+8)%12]].map(v=>[v,'分清 '+en+' 与 '+v+' 的读音。']),en,'再听一次，选出对应的词。','','Lesson11–12新词','覆盖新词；旧物品留在自由点读，不再整组必做',{audioText:en,optionImages:images});}),
  roles:[
   q('story-owner','从原文找到白衬衫的主人','课文里，谁的衬衫是白色的？',[['Tim',''],['Dave','Dave 说自己的衬衫是蓝色的。'],['老师','老师帮忙寻找主人，并没有说衬衫是自己的。']],'Tim','Dave 说 Tim’s shirt’s white.，后来 Tim 也确认衬衫是自己的。','听一听 Dave 说白衬衫的那一句。','Lesson11听前问题；L11-D09、D12–D13','完整故事理解：从描述和本人确认找到主人',{optionImages:{Tim:image('tim'),Dave:image('dave'),'老师':image('teacher')}}),
  ...
  trans:[
   ...
   q('build-sister','用关系称谓所有格回答','这件女衬衫属于我的姐姐。\n用 my sister’s 回答它是谁的。',undefined,"It's my sister's.",'my sister’s 表示“我姐姐的”；已经知道在说女衬衫，可以省去后面的 blouse。','先说 It’s，再说“我姐姐的”。','Lesson12 Written B9；Notes2','从姓名所有格进到关系称谓，允许已知物品省略',{type:'order',tokens:["It's",'my',"sister's."]})
  ],
  exam:[
   q('exam-message','听出亲属与其物品','听一听，选出对应的认领记录。',[['my brother · pen',''],['my sister · pen','物品是 pen，但录音说的物主是 my brother。'],['my brother · tie','物主是 my brother，但录音里的物品是 pen。']],'my brother · pen','录音说这是“我兄弟的钢笔”，再用 his pen 指同一件物品。','','新的完整录音；Lesson12词汇和所有格','同时听取主人和物品；不是只听一个词',{audioText:"This is my brother's pen. It's his pen."}),
 ...
 const stages=[
  {id:'l1',title:'准备找主人',activities:[['words','认领小图鉴','cards'],['listen','听音寻宝','audio']],required:['listen']},
