(function (root) {
  ...
  const modelActions=node('div','','activity-actions');nextStation('models',modelActions);surfaces.get('models').append(modelExample,modelGrid,reference,wording,modelActions);
  for(const id of ['reply','describe','trans'])mountPractice(id);
  ...
  certificateView = core.unitCertificate.mount({
    ...
    design: {
      copy: { title: '暖心的小伙伴', course: '街角问候站 · Lesson 9–10', completion: '完成 Lesson 9–10 单元练习', thanks: '认真倾听，友好问候，发现身边的小变化！' },
  ...
  writing.append(node('summary','和朋友再试试'),node('p','用虚构角色练习问候，轮流问 How are you? 并回应、回问。再选一张画卡，用 Look at… 描述图中的人或物。'));
