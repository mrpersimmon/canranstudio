(function(root){
 ...
 const REFERENCE=[
  ...
  {en:"Sophie isn't a teacher. She's a keyboard operator.",cn:'Sophie → She is → She’s',image:'/assets/unit5-6/sophie.svg'},
 ...
 const listenWords=['woman','fat','thin','tall','short','dirty','clean','hot','cold','old','young','busy','lazy'];
 const questions={
  listen:listenWords.map((en,i)=>{const others=listenWords.filter(x=>x!==en);return q('hear-'+i,'听辨 '+en,'听一听，选出单词。',[en,others[i%12],others[(i+4)%12],others[(i+8)%12]].map(v=>[v,'分清 '+en+' 与 '+v+' 的读音。']),en,'再听一次，选出对应的词。','','Lesson10新词','新词覆盖；不重刷Lesson7–8职业词',{audioText:en,optionImages:images});}),
  roles:[
  ...
  trans:[
   q('build-return','回应后也关心对方','朋友问你好不好。告诉他“很好，谢谢”，再问“你呢？”',undefined,"I'm fine, thanks. And you?",'先回应自己，再用 And you? 把问候交还给对方。','先完成 I’m fine, thanks.，再问 And you?。','L09-D04–D06合并改编','撤去完整选项，并增加回问这一回合',{type:'order',tokens:["I'm",'fine,','thanks.','And','you?']}),
  ...
  exam:[
   q('exam-message','听取人物与身体状况','听一听，选出对应的记录。',[['Tony · fine',''],['Tony · cold','录音说 very well，表示身体很好，没有说觉得冷。'],['Emma · fine','录音说的是 Tony，不是 Emma。']],'Tony · fine','Tony is very well 表示 Tony 身体很好；fine 也可以表示状态不错。','','新的完整录音','同时保留人物与状态，well与fine同义对应，不照搬上一题选项',{audioText:'Tony is very well, thanks.'}),
 ...
 const stages=[
  {id:'l1',title:'准备打招呼',activities:[['words','问候小图鉴','cards'],['listen','听音寻宝','audio']],required:['listen']},
  {id:'l2',title:'街角遇见朋友',activities:[['text','街角小剧场','book'],['roles','故事小侦探','people']],required:['text','roles']},
  {id:'l3',title:'把关心接下去',activities:[['phrases','问候小锦囊','speech'],['reply','暖心回应','heart']],required:['reply']},
