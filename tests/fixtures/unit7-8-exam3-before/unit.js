(function (root) {
  ...
  function mountPractice(id, settings = {}) {
    ...
    practice.mount({ element, questions: questions[id], sessionId: predecessors ? 'v2' : 'v' + unit.version,
      ...
      onComplete: states => {
        ...
        if (summary && predecessors) summary.textContent = { roles: '朋友资料核对好了！', reply: '采访记录填好了！', interview: '采访对话完成了！' }[id];
  ...
  writing.append(node('summary','和朋友再试试'),node('p','课堂上跟老师读词语和课文，再听老师说一个职业，找出对应图片。各选一张虚构角色卡，写上名字、国籍和职业，轮流采访；再向旁人打听这位角色。无需提供真实家庭信息。'));
