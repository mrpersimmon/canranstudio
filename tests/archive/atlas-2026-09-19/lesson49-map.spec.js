...
async function finishRealListeningStage(page) {
  ...
  for (const word of ['butcher','meat','beef','lamb','steak','mince']) {
    await room.getByRole('button',{name:'听单词',exact:true}).click();
    await room.getByRole('button',{name:word,exact:true}).click();
    await room.getByRole('button',{name:'检查答案',exact:true}).click();
    await expect(room.getByRole('status')).toContainText('未用额外提示答对');
    await room.getByRole('button',{name:'继续',exact:true}).click();
  }
  await expect(page.locator('#listenSummary')).toContainText('完成这轮听辨');
}
