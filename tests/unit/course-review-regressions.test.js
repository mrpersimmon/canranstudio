...
test('an active legacy placement retains its sample when future questions are appended',()=>{
 ...
 assert.equal(rt.snapshot().screen,'map');assert.deepEqual(rt.snapshot().record.keyboardArchive.record.placement.attempts[attempt.targetId].questionIds,attempt.questionIds);
});
test('a historical wrong answer is not regraded using an expanded answer policy',()=>{
 ...
 const changed=clone(unit);changed.placement.questions[0].hint='new version support';
 const rt=runtime.createRuntime({unit:changed,adapter:store.createMemoryAdapter({[key]:{revision:1,value:record}})});
 assert.equal(rt.snapshot().screen,'map');assert.equal(rt.snapshot().record.keyboardArchive.record.placement.attempts[attempt.targetId].responses[0].correct,false);
});
test('adding assessment metadata preserves original result evidence',()=>{
 ...
 const rt=runtime.createRuntime({unit:changed,adapter:store.createMemoryAdapter({[key]:{revision:1,value:record}})});
 assert.equal(rt.snapshot().screen,'map');assert.deepEqual(rt.snapshot().record.keyboardArchive.record.results[a.resultId].assessment,history.assessments[a.id]);
});
test('reviewed translations apply to early challenges and placement without forgiving grammar errors',()=>{
 const q=unit.keyboardHistory.challenges.find(c=>c.id==='CH21-22').questions.find(q=>q.sourceRef==='L21-E02');
 assert.ok(answers.accepts(q,'Give an empty glass to me.'));assert.ok(!answers.accepts(q,'Give me a empty glass.'));
 assert.ok(answers.accepts(unit.keyboardHistory.placement.questions.find(item=>item.id==='PL-'+q.id),'Give an empty glass to me.'));
});
...
test('migration first preserves the original bytes outside the primary record',()=>{
 ...
 assert.equal(s.data.get(key+':recovery:migration'),raw);assert.equal(rt.snapshot().record.contractVersion,3);
});
...
test('placement point selections journal small durable drafts without rewriting course progress',()=>{
 ...
 const q=placement.question(unit,a.questionIds[0]);
 for(const id of q.answer)rt.dispatch({type:'exercise-select',id,questionId:q.id});
 const response=rt.snapshot().response;
 assert.equal(s.writes.filter(w=>w.key===key).length,0);assert.ok(s.writes.length>0);assert.ok(s.writes.every(w=>w.bytes<1000));
 ...
 assert.deepEqual(restored.dispatch({type:'placement-start'}).view.response,response);
});
test('grammar pilot uses new IDs and distinguishes selection, transfer and delayed retrieval',()=>{
 assert.equal(unit.grammar?.activityIds.length,6);
 for(const id of unit.grammar.activityIds){const a=unit.activities[id];assert.equal(a.kind,'exercise');assert.ok(a.assessment.grammarSkillId);assert.equal(history.assessments[id],undefined);}
 assert.equal(unit.grammar.delayedQuestions.length,3);
 assert.equal(unit.activities['L49:grammar-correct:tap'].mechanism,'repair');
 assert.equal(unit.activities['L49:grammar-question:tap'].mechanism,'order');
...
test('wrong challenge answers survive correction and enter next-day review without mainline credit',()=>{
 ...
 tap(rt,c.questions[0],false);rt.dispatch({type:'challenge-check'});
 assert.ok(Object.keys(rt.snapshot().record.retrievalReviews||{}).length,'wrong answer is durable before correction');
 rt.dispatch({type:'challenge-retry'});tap(rt,c.questions[0]);rt.dispatch({type:'challenge-check'});rt.dispatch({type:'challenge-next'});
 ...
 tap(rt,c.questions[0]);rt.dispatch({type:'challenge-check'});v=rt.dispatch({type:'challenge-next'}).view;
...
test('an ongoing old placement is archived without scoring it as a new test',()=>{
 const {old,record,attempt}=oldPlacement();const q=placement.question(old,attempt.questionIds[0]);
 const graded=placement.grade(old,attempt,q.answers[0],'2026-09-09');record.placement={attempts:{[graded.targetId]:graded}};
 const rt=runtime.createRuntime({unit,adapter:store.createMemoryAdapter({[key]:{revision:1,value:record}})});
 assert.deepEqual(rt.snapshot().record.keyboardArchive.record.placement.attempts[graded.targetId],graded);
 assert.equal(rt.snapshot().record.placement.attempts[graded.targetId],undefined);
 rt.dispatch({type:'open-placement',id:graded.targetId});const v=rt.dispatch({type:'placement-start'}).view;
 assert.equal(v.placementAttempt.version,3);assert.equal(v.placementAttempt.responses.length,0);assert.equal(v.placementAttempt.questionIds.length,20);
});
test('two tabs cannot silently replace each other’s journal',()=>{
 ...
 const attempt=a.snapshot().placementAttempt,event={type:'exercise-select',questionId:attempt.questionIds[0]},q=placement.question(unit,attempt.questionIds[0]);
 a.dispatch({...event,id:q.options[0].id});assert.equal(b.dispatch({...event,id:q.options[1].id}).view.saveState,'conflict');
 assert.deepEqual(JSON.parse(s.data.get(key+':draft')).value.selected,[q.options[0].id]);
});
...
test('unknown placement answer policies fail validation without crashing record recovery',()=>{
 const {record,attempt}=oldPlacement();const graded=placement.grade(oldUnit(),attempt,'wrong','2026-09-09');graded.responses[0].answerPolicyVersion=999;
...
test('pilot selections preserve the actual diagnostic and routes novel sentences to later review',()=>{
 const u=clone(unit),id='L49:grammar-s:tap',a=u.activities[id];u.nodes=[{id:'only',activityIds:[id]}];
 const rt=runtime.createRuntime({unit:u,adapter:store.createMemoryAdapter(),now:()=>new Date('2026-09-09T12:00:00')});
 rt.dispatch({type:'open-node',nodeId:'only'});tap(rt,a,false);assert.equal(rt.dispatch({type:'check'}).view.feedback,'retry');
 const immediate=Object.values(rt.snapshot().record.retrievalReviews || {});assert.equal(immediate.length,1);assert.equal(immediate[0].errors[0].code,'present-third-person-s');
 ...
 rt.dispatch({type:'retry'});tap(rt,a);rt.dispatch({type:'check'});const v=rt.dispatch({type:'continue'}).view;
 assert.equal(v.record.results[a.resultId].initialEvidence,'supported');assert.deepEqual(v.record.results[a.resultId].errors[0].value.selected,['o1']);
...
test('a pilot challenge error recalls a new sentence of the same grammar skill',()=>{
 ...
 assert.equal(entry.errors[0].code,'cloze');
 ...
 tap(rt,novel);rt.dispatch({type:'challenge-check'});rt.dispatch({type:'challenge-next'});
 const saved=rt.snapshot().record.retrievalReviews[entry.questionId];assert.equal(saved.lastQuestionId,novel.id);assert.deepEqual(saved.lastAnswer,require('../../core/learning-exercises').solution(novel));
 assert.equal(saved.lastAnswerPolicyVersion,3);assert.equal(saved.lastEvidence,'independent');
 assert.deepEqual(rt.snapshot().record.challenges[c.id].answers,[]);
});

test('malformed new error histories and selection drafts reach record recovery without throwing',()=>{
 ...
 for(const extra of [{value:{}},{value:'x'.repeat(181)},{errors:[null]}]){
  const record=runtime.emptyRecord(unit);record.attempts['L49:grammar-s:tap']={wrong:1,hintUsed:true,...extra};
  assert.equal(runtime.validRecord(record,unit),false);
 }
});

test('a grandfathered completed node can earn its new tap evidence',()=>{
 const u=clone(unit),node=u.nodes.find(n=>n.id==='L49-USE');u.nodes=[node];u.checkpointIds=[node.id];
 const old={...clone(unit),...clone(unit.keyboardHistory)};delete old.contractVersion;
 old.nodes=[{...node,activityIds:clone(history.nodes[node.id])}];old.checkpointIds=[node.id];old.checkpointActivities[node.id]=clone(history.nodes[node.id]);
 ...
 const rt=runtime.createRuntime({unit:u,adapter:h.adapter});assert.equal(rt.snapshot().completedCount,1);
 const before=clone(rt.snapshot().record.results),opened=rt.dispatch({type:'open-node',nodeId:node.id}).view;
 assert.equal(opened.mode,'main');const a=u.activities[opened.activityId];assert.equal(a.kind,'exercise');
 tap(rt,a);rt.dispatch({type:'check'});rt.dispatch({type:'continue'});
 assert.equal(rt.snapshot().record.results[a.resultId].initialEvidence,'independent');
 for(const [id,result]of Object.entries(before))assert.deepEqual(rt.snapshot().record.results[id],result);
