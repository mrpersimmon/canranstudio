...
function setup(options={}) {
  ...
  const view=()=>rt.snapshot(true);
  const send=event=>{const v=rt.dispatch(event,{light:true}).view;assert.notEqual(v.screen,'blocked',JSON.stringify(event));assert.equal(v.saveState,null);assert.ok(runtime.validRecord(v.record,unit),JSON.stringify(event));assert.doesNotMatch(scene.createRenderer(unit).render(v),/src="(?:undefined)?"/);return v;};
  const hear=()=>{for(let i=0;['loading','playing'].includes(view().audio?.status)&&i<25;i++){const a=view().audio;send({type:'audio-ended',requestId:a.requestId,index:a.index});}};
