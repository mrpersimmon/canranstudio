...
function validReport(){return {schema:1,status:'passed',checks:VIEWPORTS.flatMap(viewport=>[
  ...
  ...Object.values(require('../../content/learning-course.json').activities).filter(a=>a.kind==='exercise').flatMap(a=>['tap-selected-','tap-correct-'].map(prefix=>({name:prefix+a.id,activityId:a.id,viewport,expectedTheme:'dark',canvas:'rgb(20, 31, 35)',errors:[]}))),
...
test('visual release gate fails closed on errors, missing screens, missing widths and ineffective negative controls',()=>{
  ...
  for(const mutate of [
    ...
    p=>{p.checks=p.checks.filter(c=>c.name!=='challenge-filled-'+require('../../content/learning-course.json').challenges[0].questions[0].id);}
