...
const {completeActivity,completeStory,completeUnit2728}=require('../support/unit27-28-flow');
const fs=require('node:fs/promises');
test.use({reducedMotion:'reduce',actionTimeout:5000});

test('挑战错答与提示分别记账，暂停刷新保留选择，不提前发证',async({page})=>{
  await page.goto('/unit27-28/#learn/exam');const room=page.locator('.stage-exam'),check=room.getByRole('button',{name:'检查答案',exact:true});
...
test('lesson路径阻断全部声音仍可完整通关和领证，各单元与根路径记录互不串用',async({page})=>{
  ...
  await completeUnit2728(page,'/lesson');await page.getByRole('button',{name:'领取单元证书',exact:true}).click();
  await expect(page.getByRole('dialog',{name:'客厅发现之旅纪念',exact:true})).toBeVisible();await page.keyboard.press('Escape');await page.reload();await expect(page.locator('#starCount')).toHaveText('15');
  await page.getByRole('link',{name:'我的课程',exact:true}).click();await expect(page).toHaveURL(/\/lesson\/$/);
  await expect(page.getByRole('link',{name:'继续学习：客厅发现之旅',exact:true})).toHaveAttribute('href','/lesson/unit27-28/#learn/certificate');
  for(const unit of ['unit1-2','unit3-4','unit5-6','unit7-8','unit9-10','unit11-12','unit13-14','unit15-16','unit17-18','unit19-20','unit21-22','unit23-24','unit25-26','unit49-50']){
    await page.goto('/lesson/'+unit+'/#learn/certificate');await expect(page.locator('#starCount')).toHaveText('0');
  }
  await page.goto('/unit27-28/#learn/certificate');await expect(page.locator('#starCount')).toHaveText('0');expect(voices).toEqual([]);
});

test('首页续学、同一位置继续冒险、重开取消与确认均遵守网站路径隔离',async({page})=>{
  await page.goto('/unit27-28/#learn/observe');const room=page.locator('.stage-observe');await room.getByRole('button',{name:"are",exact:true}).click();
  ...
  await page.getByRole('link',{name:'我的课程',exact:true}).click();await page.getByRole('link',{name:'继续学习：客厅发现之旅',exact:true}).click();
  await expect(page).toHaveURL(/#learn\/observe$/);await expect(room.getByRole('button',{name:"are",exact:true})).toHaveAttribute('aria-pressed','true');
  await page.goto('/lesson/unit27-28/#learn/words');await page.locator('.stage-words').getByRole('button',{name:'下一组词卡',exact:true}).click();
  ...
  for(const confirm of [false,true]){
    ...
    if(!confirm){await page.getByRole('button',{name:'返回课程',exact:true}).click();await expect(page.getByRole('link',{name:'继续学习：客厅发现之旅',exact:true})).toBeVisible();}
  }
  await expect(page.getByRole('link',{name:'开始学习：客厅发现之旅',exact:true})).toHaveAttribute('href','/lesson/unit27-28/#learn/words');
  await page.getByRole('link',{name:'开始学习：客厅发现之旅',exact:true}).click();await expect(page.locator('#wordPageProgress')).toHaveText('1 / 5');
  await page.goto('/unit27-28/#learn/observe');await expect(room.getByRole('button',{name:"are",exact:true})).toHaveAttribute('aria-pressed','true');
});
...
test('更新一道理解题只失效对应活动，不清除词卡位置、其他练习和拼句草稿',async({page})=>{
  ...
  await page.goto('/unit27-28/#learn/trans');await expect(page.locator('.stage-trans').getByRole('button',{name:'撤回 There',exact:true})).toBeVisible();
  await page.goto('/unit27-28/#learn/observe');await expect(page.locator('.stage-observe').getByRole('group',{name:'完成后的操作',exact:true})).toBeVisible();
...
test('保存失败如实提示，恢复存储后可重试，刷新仍保留真实作答',async({page})=>{
  await page.addInitScript(()=>{
    ...
    Storage.prototype.setItem=function(key,value){if(key==='canran:unit27-28:learning:v1'&&window.testStorageBlocked)throw new DOMException('blocked','QuotaExceededError');return Reflect.apply(set,this,[key,value]);};
  });
  await page.goto('/unit27-28/#learn/observe');const room=page.locator('.stage-observe');
  await room.getByRole('button',{name:'are',exact:true}).click();await room.getByRole('button',{name:'检查答案',exact:true}).click();await expect(room.getByRole('status')).toContainText('答对了！');
