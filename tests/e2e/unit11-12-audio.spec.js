...
for(const base of ['', '/lesson'])test(`unit11-12 词卡翻面、示范阅读和刷新都不请求英语配音 ${base||'/'}`,async({page})=>{
 ...
 for(let i=0;i<18;i+=6){
  ...
  if(i+6<18)await room.getByRole('button',{name:'下一组词卡',exact:true}).click();
 }
 await page.reload();await expect(room.locator('.word-phonetic').first()).toBeVisible();
 for(const activity of ['phrases','models']){await page.goto(base+'/unit11-12/#learn/'+activity);const section=page.locator('.stage-'+activity);for(const summary of await section.locator('summary').all())await summary.click();await expect(section.locator('button.phrase-card')).toHaveCount(0);await expect(section.locator('article.phrase-card')).not.toHaveCount(0);}
