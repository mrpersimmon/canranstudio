...
const KEY='canran:l52:progress:v2';
...
test('Lesson 52 exposes an actionable locked certificate from zero stars',async({page})=>{
  await page.goto('/lesson52/#cert');
...
test('Lesson 52 jumps to its first not-full-star passport stop',async({page})=>{
...
test('Lesson 52 blocks fourteen stars and prints at fifteen',async({page})=>{
...
test('Lesson 52 rechecks eligibility at the delayed print boundary',async({page})=>{
...
test('Lesson 52 keeps only one pending delayed print',async({page})=>{
  await seed(page,{l1:3,l2:3,l3:3,l4:3,l5:3});
  await controlCertificatePrintTimer(page);
  await page.goto('/lesson52/#cert');
  await page.locator('#certName').fill('小明');
