...
test('从词卡开始，浏览16句并完成32题后才获得15星课堂配套证书',async({page})=>{
...
test('首页继续定位与旧 be、trans 书签衔接到合并活动',async({page})=>{
 await page.goto('/lesson/');await page.getByRole('link',{name:'开始学习：新朋友采访站',exact:true}).click();await expect(page).toHaveURL(/\/lesson\/unit7-8\/#learn\/words$/);
 await page.goto('/lesson/unit7-8/#learn/be');await expect(page).toHaveURL(/#learn\/reply$/);const room=page.locator('.stage-reply');
 await room.getByRole('button',{name:"I'm Italian.",exact:true}).click();await page.locator('#startBtn').scrollIntoViewIfNeeded();await page.getByRole('button',{name:'继续冒险',exact:true}).click();await expect(room.getByRole('heading').first()).toBeInViewport();
 await page.getByRole('link',{name:'我的课程',exact:true}).click();const resume=page.getByRole('link',{name:'继续学习：新朋友采访站',exact:true});await expect(resume).toHaveAttribute('href','/lesson/unit7-8/#learn/reply');await resume.click();await expect(room.getByRole('button',{name:"I'm Italian.",exact:true})).toHaveAttribute('aria-pressed','true');
 await page.goto('/unit7-8/#learn/trans');await expect(page).toHaveURL(/#learn\/interview$/);await expect(page.locator('.stage-interview .interview-task-prompt')).toContainText('Robert');await expect(page.locator('.shop-stage')).toHaveCount(10);
});
...
test('挑战暂停保留选择，记录区分独立、提示和修正；不提前发证',async({page})=>{
 ...
 await room.getByRole('button',{name:'给点线索',exact:true}).click();await room.getByRole('button',{name:'Are you a mechanic?',exact:true}).click();await room.getByRole('button',{name:'暂停，稍后继续',exact:true}).click();await page.reload();await room.getByRole('button',{name:'继续挑战',exact:true}).click();await expect(room.getByRole('button',{name:'Are you a mechanic?',exact:true})).toHaveAttribute('aria-pressed','true');await check.click();await room.getByRole('button',{name:'下一题',exact:true}).click();for(const token of ['My',"name's",'Ben.',"I'm",'an engineer.'])await room.getByRole('group',{name:'待选词块',exact:true}).getByRole('button',{name:token,exact:true}).click();await check.click();await room.getByRole('button',{name:'下一题',exact:true}).click();await require('../support/unit7-8-exam').finishExamFrom(page,3);await expect(room).toContainText('首次独立答对 8 / 10');await expect(room).toContainText('提示后完成 1 题 · 修正后完成 1 题');await page.goto('/unit7-8/#learn/certificate');await expect(page.locator('#starCount')).toHaveText('3');await expect(page.getByRole('button',{name:'领取单元证书',exact:true})).toBeDisabled();
});

test('内容升版只重置变动活动，不从旧题或历史成绩重新填答案',async({page})=>{
 await page.goto('/unit7-8/#learn/reply');const reply=page.locator('.stage-reply');await reply.getByRole('button',{name:"I'm Italian.",exact:true}).click();await reply.getByRole('button',{name:'检查答案',exact:true}).click();
 await page.goto('/unit7-8/#learn/interview');const interview=page.locator('.stage-interview');await interview.getByRole('button',{name:"What's his job?",exact:true}).click();
 await page.route('**/unit7-8/content.js*',async route=>{const response=await route.fetch();await route.fulfill({response,body:(await response.text()).replace("q('reply-nationality'","q('reply-nationality-revised'")});});await page.reload();await expect(interview.getByRole('button',{name:"What's his job?",exact:true})).toHaveAttribute('aria-pressed','true');
 await page.goto('/unit7-8/#learn/reply');await expect(reply.getByRole('button',{name:"I'm Italian.",exact:true})).toHaveAttribute('aria-pressed','false');await expect(reply.getByRole('progressbar')).toHaveAttribute('aria-valuenow','0');await expect(reply.getByRole('button',{name:'检查答案',exact:true})).toBeDisabled();
});

test('首页重开包含新单元，取消保留、确认仅清除 lesson 路径记录',async({page})=>{
 await page.goto('/unit7-8/#learn/reply');const root=page.locator('.stage-reply');await root.getByRole('button',{name:"I'm Italian.",exact:true}).click();for(const path of ['unit1-2/#learn/ask','unit3-4/#learn/reply','unit5-6/#learn/refer','unit7-8/#learn/reply','unit49-50/#learn/give']){await page.goto('/lesson/'+path);await expect(page.locator('.stage-'+path.split('#learn/')[1])).toBeVisible();}await page.goto('/lesson/');await page.getByRole('button',{name:'设备冒险设置',exact:true}).click();await page.getByRole('button',{name:'重开冒险',exact:true}).click();await page.getByRole('button',{name:'继续确认',exact:true}).click();await page.getByRole('button',{name:'取消重开',exact:true}).click();await page.getByRole('button',{name:'返回课程',exact:true}).click();await expect(page.getByRole('link',{name:'继续学习：新朋友采访站',exact:true})).toBeVisible();await page.getByRole('button',{name:'设备冒险设置',exact:true}).click();await page.getByRole('button',{name:'重开冒险',exact:true}).click();await page.getByRole('button',{name:'继续确认',exact:true}).click();await page.getByRole('button',{name:'确认重开',exact:true}).click();for(const name of ['礼貌小帮手','雨伞认领小帮手','新朋友见面会','新朋友采访站'])await expect(page.getByRole('link',{name:'开始学习：'+name,exact:true})).toBeVisible();await page.goto('/unit7-8/#learn/reply');await expect(root.getByRole('button',{name:"I'm Italian.",exact:true})).toHaveAttribute('aria-pressed','true');
});
