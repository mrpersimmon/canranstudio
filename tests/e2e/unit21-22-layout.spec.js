...
const {ANSWERS}=require('../support/unit21-22-flow');
...
for(const width of [320,390,768,1280])test(width+' 各站完整、无横向溢出，帮助返回焦点，所有资源加载',async({page})=>{
  ...
  for(const id of ['words','listen','text','roles','phrases','observe','be','models','trans','exam','certificate']){
    await page.goto('/unit21-22/#learn/'+id);const room=page.locator('.stage-'+id);
    ...
    if([390,1280].includes(width)&&['words','listen','text','observe','be','models','trans'].includes(id))await page.screenshot({path:'output/playwright/unit21-22/'+id+'-'+width+'.png'});
  }
  expect(errors).toEqual([]);
});

test('320 窄屏36张词卡展开后字义和音标不裁切，图册成对对比完整可辨',async({page})=>{
  await page.setViewportSize({width:320,height:740});await page.goto('/unit21-22/#learn/words');const room=page.locator('.stage-words');
  for(let p=0;p<6;p++){
  ...
  await page.goto('/unit21-22/#learn/models');await page.getByText('看看十六幅物品图',{exact:true}).click();const gallery=page.locator('.comparison-gallery');
  await gallery.screenshot({path:'output/playwright/unit21-22/comparison-gallery-320.png'});
});

for(const width of [320,1280])test(width+' 灯泡位于检查左边，反馈不推移操作，结束按钮为居中的主次双按钮',async({page})=>{
  await page.setViewportSize({width,height:740});await page.goto('/unit21-22/#learn/observe');await page.evaluate(()=>document.fonts.ready);
  ...
  await room.getByRole('button',{name:"只有说话的人",exact:true}).click();await check.click();
  ...
  await room.screenshot({path:'output/playwright/unit21-22/finish-'+width+'.png'});
});

test('390短屏阅读内部滚动，下一句与重新上演同行，展开中文不移动推进按钮',async({page})=>{
  await page.setViewportSize({width:390,height:500});await page.goto('/unit21-22/#learn/text');await page.evaluate(()=>document.fonts.ready);const room=page.locator('.stage-text');
  ...
  await room.screenshot({path:'output/playwright/unit21-22/story-short-screen.png'});
});

test('词块可撤回、键盘操作、刷新续填；错序不能过关，新题不继承答案',async({page})=>{
  await page.goto('/unit21-22/#learn/trans');const room=page.locator('.stage-trans'),bank=room.getByRole('group',{name:'待选词块',exact:true}),selected=room.getByRole('group',{name:'已选词块',exact:true});
  const order=await bank.getByRole('button').allTextContents();await bank.getByRole('button',{name:'Give',exact:true}).click();await selected.getByRole('button',{name:'撤回 Give',exact:true}).click();
  for(const token of ['please.','clean cup,','a','her','Give'])await bank.getByRole('button',{name:token,exact:true}).click();
...
test('两个 one? 词块独立选取撤回，刷新不会替孩子补选',async({page})=>{
 const {chooseTokens}=require('../support/unit21-22-flow');await page.goto('/unit21-22/#learn/trans');const room=page.locator('.stage-trans'),bank=room.getByRole('group',{name:'待选词块',exact:true}),selected=room.getByRole('group',{name:'已选词块',exact:true});
 await chooseTokens(room,ANSWERS.trans[0]);await room.getByRole('button',{name:'检查答案',exact:true}).click();await room.getByRole('button',{name:'下一题',exact:true}).click();
 await expect(selected.getByRole('button')).toHaveCount(0);await expect(room.getByRole('button',{name:'检查答案',exact:true})).toBeDisabled();await expect(bank.getByRole('button',{name:'one?',exact:true})).toHaveCount(2);
 await chooseTokens(room,ANSWERS.trans[1]);await expect(selected.getByRole('button',{name:'撤回 one?',exact:true})).toHaveCount(2);await selected.getByRole('button',{name:'撤回 one?',exact:true}).last().click();await page.reload();
 await expect(selected.getByRole('button')).toHaveCount(4);for(const [i,name] of ['撤回 Which','撤回 one?','撤回 This','撤回 blue'].entries())await expect(selected.getByRole('button').nth(i)).toHaveAccessibleName(name);await expect(room.getByRole('button',{name:'检查答案',exact:true})).toBeDisabled();await chooseTokens(room,['one?']);await room.getByRole('button',{name:'检查答案',exact:true}).click();await expect(room.getByRole('status')).toContainText('答对了！');await expect(room.getByRole('progressbar')).toHaveAttribute('aria-valuenow','2');
});

test('320 最后一道长拼句和挑战选项完整可读，不因完成上一题而预选',async({page})=>{
 const {chooseTokens}=require('../support/unit21-22-flow');await page.setViewportSize({width:320,height:740});await page.goto('/unit21-22/#learn/trans');let room=page.locator('.stage-trans');
