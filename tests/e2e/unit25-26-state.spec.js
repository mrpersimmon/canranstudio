...
const {completeActivity,completeStory,completeUnit2526}=require('../support/unit25-26-flow');
...
test('挑战错答与提示分别记账，暂停刷新保留选择，不提前发证',async({page})=>{
  ...
  await page.goto('/unit25-26/#learn/certificate');await expect(page.locator('#starCount')).toHaveText('3');await expect(page.getByRole('button',{name:'领取单元证书',exact:true})).toBeDisabled();
});
...
test('正误和完成反馈音实际播放结束，任何英语声音请求和系统朗读均未发生',async({page})=>{
  ...
  await room.getByRole('button',{name:'下一题',exact:true}).click();await room.getByRole('button',{name:"杯子很干净。",exact:true}).click();await check.click();await expect(room.getByRole('status')).toHaveText('再看看，试一次。');
  await room.getByRole('button',{name:'再试一次',exact:true}).click();await room.getByRole('button',{name:"桌上有杯子。",exact:true}).click();await check.click();await room.getByRole('button',{name:'查看本次记录',exact:true}).click();
...
test('lesson路径阻断全部声音仍可完整通关和领证，各单元与根路径记录互不串用',async({page})=>{
  ...
  await completeUnit2526(page,'/lesson');await page.getByRole('button',{name:'领取单元证书',exact:true}).click();
  await expect(page.getByRole('dialog',{name:'厨房探访记纪念',exact:true})).toBeVisible();await page.keyboard.press('Escape');await page.reload();await expect(page.locator('#starCount')).toHaveText('15');
  await page.getByRole('link',{name:'我的课程',exact:true}).click();await expect(page).toHaveURL(/\/lesson\/$/);
  await expect(page.getByRole('link',{name:'继续学习：厨房探访记',exact:true})).toHaveAttribute('href','/lesson/unit25-26/#learn/certificate');
  for(const unit of ['unit1-2','unit3-4','unit5-6','unit7-8','unit9-10','unit11-12','unit13-14','unit15-16','unit17-18','unit19-20','unit21-22','unit23-24','unit49-50']){
    await page.goto('/lesson/'+unit+'/#learn/certificate');await expect(page.locator('#starCount')).toHaveText('0');
  }
  await page.goto('/unit25-26/#learn/certificate');await expect(page.locator('#starCount')).toHaveText('0');expect(voices).toEqual([]);
});
...
test('首页续学、同一位置继续冒险、重开取消与确认均遵守网站路径隔离',async({page})=>{
  ...
  for(const confirm of [false,true]){
    ...
    if(!confirm){await page.getByRole('button',{name:'返回课程',exact:true}).click();await expect(page.getByRole('link',{name:'继续学习：厨房探访记',exact:true})).toBeVisible();}
  }
...
test('更新一道理解题只失效对应活动，不清除词卡位置、其他练习和拼句草稿',async({page})=>{
  ...
  await page.route('**/unit25-26/content.js*',async route=>route.fulfill({contentType:'text/javascript',body:(await fs.readFile('unit25-26/content.js','utf8')).replace("q('story-colour',","q('story-colour-v2',")}));
...
test('关闭JavaScript首页仍提供新单元真实链接',async({browser})=>{
  ...
  await page.goto('http://127.0.0.1:4173/lesson/');await expect(page.getByRole('link',{name:'开始学习：厨房探访记',exact:true})).toHaveAttribute('href','/lesson/unit25-26/#learn/words');await context.close();
});
