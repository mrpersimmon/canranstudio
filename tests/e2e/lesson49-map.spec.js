...
async function finishRealListeningStage(page) {
  ...
  for (let round = 0; round < 8; round += 1) {
    ...
    await page.locator('#lgOpts .opt-btn').filter({ hasText: answer }).click();
    if (round < 7) {
      await expect(page.locator('#lgRound')).toHaveText(`第 ${round + 2} / 8 轮`, { timeout: 2500 });
    }
  }
...
test('Lesson 49 recommendation, real-stage growth, souvenir persistence, and restart form one loop', async ({ page }) => {
  ...
  await expect(landmark.locator('[data-landmark-layer]')).toHaveCount(2);
  await expect(landmark.locator('[data-growth-stage="l2"]')).toBeVisible();
  ...
  await expect(landmark.locator('[data-landmark-layer]')).toHaveCount(3);
  await expect(landmark.locator('[data-growth-stage="l1"]')).toBeVisible();
  await expect(landmark.locator('[data-growth-stage="l2"]')).toBeVisible();
  ...
  await expect(landmark.locator('[data-landmark-layer]')).toHaveCount(6);
