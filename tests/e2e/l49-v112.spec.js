...
test('错答只鼓励重试，刷新不泄题，答对后简短确认', async ({ page }) => {
  ...
  await expect(feedback.getByText(explanation, { exact: true })).toHaveCount(0);
...
test('分拣题答对只显示成功，答错只提示原题重试', async ({ page }) => {
...
for (const width of [320, 390, 768, 1280]) test(`${width}px 无独立线索时隐藏灯泡，检查和重试位置稳定`, async ({ page }) => {
...
test('词块拼装错答不泄露词序，重拼保留真实首答记录', async ({ page }) => {
...
test('320px 锦囊错答仅提示重试，反馈区不滚动', async ({ page }) => {
