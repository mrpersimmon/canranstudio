...
const {ANSWERS}=require('../support/unit15-16-flow');
...
for(const width of [320,390,768,1280])test(width+' 各站完整、无横向溢出，帮助返回焦点，所有资源加载',async({page})=>{
  ...
  for(const id of ['words','listen','text','roles','phrases','reply','forms','models','trans','exam','certificate']){
    await page.goto('/unit15-16/#learn/'+id);const room=page.locator('.stage-'+id);
    ...
    if([390,1280].includes(width)&&['words','listen','text','reply','forms','models','trans'].includes(id))await page.screenshot({path:'output/playwright/unit15-16/'+id+'-'+width+'.png'});
  }
  expect(errors).toEqual([]);
});

test('320 窄屏25张词卡展开后字义和音标不裁切，图册双色物品完整可辨',async({page})=>{
  await page.setViewportSize({width:320,height:740});await page.goto('/unit15-16/#learn/words');const room=page.locator('.stage-words');
  for(let p=0;p<5;p++){
...
for(const width of [320,1280])test(width+' 灯泡位于检查左边，反馈不推移操作，结束按钮为居中的主次双按钮',async({page})=>{
  await page.setViewportSize({width,height:740});await page.goto('/unit15-16/#learn/reply');await page.evaluate(()=>document.fonts.ready);
  const room=page.locator('.stage-reply'),actions=room.getByRole('group',{name:'作答操作',exact:true}),check=room.getByRole('button',{name:'检查答案',exact:true}),hint=room.getByRole('button',{name:'给点线索',exact:true});
  ...
  await room.getByRole('button',{name:'Yes, I am.',exact:true}).click();await check.click();
  ...
  await room.screenshot({path:'output/playwright/unit15-16/finish-'+width+'.png'});
});

test('390短屏阅读内部滚动，下一句与重新上演同行，展开中文不移动推进按钮',async({page})=>{
  await page.setViewportSize({width:390,height:500});await page.goto('/unit15-16/#learn/text');await page.evaluate(()=>document.fonts.ready);const room=page.locator('.stage-text');
  ...
  await room.screenshot({path:'output/playwright/unit15-16/story-short-screen.png'});
});

test('词块可撤回、键盘操作、刷新续填；错序不能过关，新题不继承答案',async({page})=>{
  await page.goto('/unit15-16/#learn/trans');const room=page.locator('.stage-trans'),bank=room.getByRole('group',{name:'待选词块',exact:true}),selected=room.getByRole('group',{name:'已选词块',exact:true});
  const order=await bank.getByRole('button').allTextContents();await bank.getByRole('button',{name:'Are',exact:true}).click();await selected.getByRole('button',{name:'撤回 Are',exact:true}).click();
  for(const token of ['these','Are','your','cases?'])await bank.getByRole('button',{name:token,exact:true}).click();
