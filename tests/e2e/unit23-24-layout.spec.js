...
const {ANSWERS}=require('../support/unit23-24-flow');
...
for(const width of [320,390,768,1280])test(width+' 各站完整、无横向溢出，帮助返回焦点，所有资源加载',async({page})=>{
  ...
  for(const id of ['words','listen','text','roles','phrases','observe','be','models','trans','exam','certificate']){
    await page.goto('/unit23-24/#learn/'+id);const room=page.locator('.stage-'+id);
    ...
    if([390,1280].includes(width)&&['words','listen','text','observe','be','models','trans'].includes(id))await page.screenshot({path:'output/playwright/unit23-24/'+id+'-'+width+'.png'});
  }
  expect(errors).toEqual([]);
});

test('320 窄屏30张词卡展开后字义和音标不裁切，图册成对对比完整可辨',async({page})=>{
  await page.setViewportSize({width:320,height:740});await page.goto('/unit23-24/#learn/words');const room=page.locator('.stage-words');
  for(let p=0;p<5;p++){
  ...
  await page.goto('/unit23-24/#learn/models');await page.getByText('看看十幅位置图',{exact:true}).click();const gallery=page.locator('.comparison-gallery');
  await gallery.screenshot({path:'output/playwright/unit23-24/comparison-gallery-320.png'});
});

for(const width of [320,1280])test(width+' 灯泡位于检查左边，反馈不推移操作，结束按钮为居中的主次双按钮',async({page})=>{
  await page.setViewportSize({width,height:740});await page.goto('/unit23-24/#learn/observe');await page.evaluate(()=>document.fonts.ready);
  ...
  await room.getByRole('button',{name:"Give me a book, please.",exact:true}).click();await check.click();
  ...
  await room.screenshot({path:'output/playwright/unit23-24/finish-'+width+'.png'});
});

test('390短屏阅读内部滚动，下一句与重新上演同行，展开中文不移动推进按钮',async({page})=>{
  await page.setViewportSize({width:390,height:500});await page.goto('/unit23-24/#learn/text');await page.evaluate(()=>document.fonts.ready);const room=page.locator('.stage-text');
  ...
  await room.screenshot({path:'output/playwright/unit23-24/story-short-screen.png'});
});

test('词块可撤回、键盘操作、刷新续填；错序不能过关，新题不继承答案',async({page})=>{
  await page.goto('/unit23-24/#learn/trans');const room=page.locator('.stage-trans'),bank=room.getByRole('group',{name:'待选词块',exact:true}),selected=room.getByRole('group',{name:'已选词块',exact:true});
  ...
  for(const token of ['please.','boxes,','some','us','Give'])await bank.getByRole('button',{name:token,exact:true}).click();
...
test('320 最后一道长拼句和挑战选项完整可读，不因完成上一题而预选',async({page})=>{
 const {chooseTokens}=require('../support/unit23-24-flow');await page.setViewportSize({width:320,height:740});await page.goto('/unit23-24/#learn/trans');let room=page.locator('.stage-trans');
 ...
 await room.screenshot({path:'output/playwright/unit23-24/long-options-320.png'});
});

test('位置图画布上沿和尺寸一致，不因说明换行而上下错位',async({page})=>{
 for(const width of [320,390,1280]){
  await page.setViewportSize({width,height:740});await page.goto('/unit23-24/#learn/models');await page.getByText('看看十幅位置图',{exact:true}).click();await page.evaluate(()=>document.fonts.ready);
  const pictures=page.locator('.comparison-gallery .phrase-card>img');await expect(pictures).toHaveCount(10);
  for(let i=0;i<10;i+=2){const a=await pictures.nth(i).boundingBox(),b=await pictures.nth(i+1).boundingBox();expect(Math.abs(a.y-b.y)).toBeLessThan(1);expect(a.height).toBe(b.height);expect(a.width).toBe(b.width);}
 }
});
