...
test('课程卡片显示各自星星，不把已完成单课冒充组合单元完成', async ({ page }) => {
  ...
  await expect(page.locator('[data-course="lesson50"]')).toContainText('5 / 15 颗星');
  await expect(page.locator('[data-course="soundmark"]')).toContainText('12 / 12 颗星');
...
test('单课历史星星在刷新后保留，导航不会清除练习手记', async ({ page }) => {
  ...
  await expect(page.locator('[data-course="lesson49"]')).toContainText('5 / 15 颗星');
