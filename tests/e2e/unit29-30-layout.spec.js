...
test('320 窄屏30张词卡展开后字义和音标不裁切，图册成对对比完整可辨',async({page})=>{
  await page.setViewportSize({width:320,height:740});await page.goto('/unit29-30/#learn/words');const room=page.locator('.stage-words');
  for(let p=0;p<5;p++){
  ...
  await page.goto('/unit29-30/#learn/models');await page.getByText('看看35个动作搭配',{exact:true}).click();const gallery=page.locator('.action-family').first();
  await gallery.screenshot({path:'output/playwright/unit29-30/comparison-gallery-320.png'});
});

for(const width of [320,1280])test(width+' 灯泡位于检查左边，反馈不推移操作，结束按钮为居中的主次双按钮',async({page})=>{
  await page.setViewportSize({width,height:740});await page.goto('/unit29-30/#learn/observe');await page.evaluate(()=>document.fonts.ready);
  ...
  await room.getByRole('button',{name:"我的东西在哪里？",exact:true}).click();await check.click();
  ...
  await room.screenshot({path:'output/playwright/unit29-30/finish-'+width+'.png'});
});

test('390短屏阅读内部滚动，下一句与从头看同行，展开中文不移动推进按钮',async({page})=>{
  await page.setViewportSize({width:390,height:500});await page.goto('/unit29-30/#learn/text');await page.evaluate(()=>document.fonts.ready);const room=page.locator('.stage-text');
  ...
  await room.screenshot({path:'output/playwright/unit29-30/story-short-screen.png'});
});

test('词块可撤回、键盘操作、刷新续填；错序不能过关，新题不继承答案',async({page})=>{
  await page.goto('/unit29-30/#learn/trans');const room=page.locator('.stage-trans'),bank=room.getByRole('group',{name:'待选词块',exact:true}),selected=room.getByRole('group',{name:'已选词块',exact:true});
  ...
  for(const token of ['do?','I','must','What'])await bank.getByRole('button',{name:token,exact:true}).click();
...
test('320 最后一道长拼句和挑战选项完整可读，不因完成上一题而预选',async({page})=>{
 const {chooseTokens}=require('../support/unit29-30-flow');await page.setViewportSize({width:320,height:740});await page.goto('/unit29-30/#learn/trans');let room=page.locator('.stage-trans');
