...
const { EXAM, selectAnswer, finishExamFrom } = require('../support/unit11-12-exam');
const oldFlow = require('../fixtures/unit11-12-short-exam-before/flow');
test.use({ reducedMotion: 'reduce', actionTimeout: 5000 });
async function oldEdition(page) {
  let old = true;
  for (const name of ['index.html', 'content.js', 'unit.js', 'unit.css', 'scene.js']) {
    const body = await fs.readFile('tests/fixtures/unit11-12-short-exam-before/' + name);
...
test('11–12 三题旧挑战完成后保留有效题，新增题亲自补完才重新领证', async ({ page }) => {
  test.setTimeout(90000); const upgrade = await oldEdition(page); await oldFlow.completeUnit1112(page);
  await page.getByRole('textbox', { name: '证书上的名字', exact: true }).fill('认领小侦探');
  ...
  await page.getByRole('button', { name: '继续：归还小挑战', exact: true }).click(); const room = page.locator('.stage-exam');
  ...
  await page.goto('/unit11-12/#learn/exam'); await room.getByRole('button', { name: '再练一轮', exact: true }).click(); await page.reload();
...
test('11–12 旧草稿只是选择，升级、暂停与刷新都不代为判对', async ({ page }) => {
  const upgrade = await oldEdition(page); await page.goto('/unit11-12/#learn/exam'); const room = page.locator('.stage-exam');
...
test('11–12 缓存版断网刷新保留新增题的未提交词块，下一题仍为空', async ({ page, context }) => {
  test.setTimeout(60000); await page.goto('/lesson/unit11-12/#learn/exam'); const room = page.locator('.stage-exam');
  for (let i = 0; i < 8; i++) {
  ...
  await room.getByRole('group', { name: '待选词块', exact: true }).getByRole('button', { name: 'Whose', exact: true }).click();
  await context.setOffline(true); const response = await page.reload(); expect(response.headers()['x-course-offline']).toBe('1');
  await expect(room.getByRole('button', { name: '撤回 Whose', exact: true })).toBeVisible();
  await expect(room.getByRole('progressbar')).toHaveAttribute('aria-valuenow', '8'); await expect(room.getByRole('status')).toBeEmpty();
  for (const token of ['shirt', 'is', 'that?']) await room.getByRole('group', { name: '待选词块', exact: true }).getByRole('button', { name: token, exact: true }).click();
  await room.getByRole('button', { name: '检查答案', exact: true }).click(); await room.getByRole('button', { name: '下一题', exact: true }).click();
