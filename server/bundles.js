...
async function retainPackages(packages,directory){
 const root=path.join(directory,'course-bundles'),objects=path.join(root,'objects');await fs.mkdir(objects,{recursive:true,mode:0o700});
 for(const descriptor of Object.values(packages.index.courses)){
  const pack=JSON.parse(packages.generated.get(descriptor.manifest.slice(8)).body);
  for(const item of [...pack.required,...pack.audio]){const file=path.join(objects,item.sha256);try{await fs.writeFile(file,packages.generated.get(item.url.slice(8)).body,{flag:'wx',mode:0o600});}catch(error){if(error.code!=='EEXIST')throw error;}}
 ...
 for(const name of await fs.readdir(root)){
  ...
  if(pack.accessPolicy!=='class-v1'||!UNITS.includes(pack.id))continue;
  for(const item of [...pack.required,...pack.audio]){
   if(!/^[a-f0-9]{64}$/.test(item.sha256)||!item.url.startsWith('/lesson/resources/'+item.sha256+'/'))throw Error('Invalid private course history');
   retained.add(item.sha256);const key=item.url.slice(8);if(!packages.generated.has(key))packages.generated.set(key,{body:await fs.readFile(path.join(objects,item.sha256)),type:item.type});
  }
