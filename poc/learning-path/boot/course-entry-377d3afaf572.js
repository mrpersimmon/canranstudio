(function bootCoursePackageEntry(global) {
  ...
  async function loadUnitCatalog() {
    ...
    const unit = global.CanranCore.courseCatalogWire.decode(await response.json());
    const expectedRevision = state.result?.manifest?.revision;
    if (
      unit?.unitId !== root.dataset.unitId
      || (expectedRevision && (unit?.releaseRevision || unit?.experienceRevision) !== expectedRevision)
    ) {
  ...
  async function enterCourse() {
    ...
    try {
      ...
      if (catalog.TEACHING_UNITS[0].runtimeProfile === 'learning-path-v3' || root.dataset.unitId === 'NCE-U01') {
