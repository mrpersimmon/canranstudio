...
> 2026-08-05 美术实现补充：本文早期章节中的“底图 + 独立成长图层”已经被真机视觉审查否决。当前权威合同是 `docs/designs/adventure-map/landmark-art-bible-v1.md`：每个地点必须从同一高清母版逐步编辑为完整累计 `state-0…N`，运行时只切换完整状态图，禁止恢复松散图层组合。其余入口、进度、揭晓时机与 V1 边界继续有效。
